  ...
  return (
    <div className="group rounded-2xl border border-black/10 bg-white/60 p-5 shadow-sm shadow-black/5 backdrop-blur transition hover:-translate-y-0.5 hover:border-black/20 hover:shadow-md hover:shadow-black/10 dark:border-white/10 dark:bg-white/5 dark:shadow-black/40 dark:hover:border-white/20">
      <div className="mb-4 overflow-hidden rounded-xl border border-black/5 bg-gradient-to-br from-blue-50 via-white to-indigo-50 p-3 dark:border-white/10 dark:from-blue-950/40 dark:via-slate-900/30 dark:to-indigo-950/40">
        <div className="text-micro flex items-center justify-between text-zinc-600 dark:text-zinc-300">
          <span className="text-micro rounded-full border border-black/10 bg-white/70 px-2 py-0.5 font-medium text-zinc-700 backdrop-blur dark:border-white/10 dark:bg-white/10 dark:text-zinc-200">
            Featured
          </span>
          {project.language ? (
            <span className="text-micro rounded-full border border-black/10 bg-white/70 px-2 py-0.5 font-medium text-zinc-700 backdrop-blur dark:border-white/10 dark:bg-white/10 dark:text-zinc-200">
        ...
        <div className="mt-3 flex items-center gap-3">
          <div className="text-body flex h-10 w-10 items-center justify-center rounded-full bg-white/90 font-semibold text-indigo-700 shadow-sm dark:bg-white/10 dark:text-indigo-200">
            {project.name.slice(0, 2).toUpperCase()}
          </div>
          <div className="min-w-0">
            <div className="text-body truncate font-semibold text-zinc-900 dark:text-white">
              {project.name}
            </div>
